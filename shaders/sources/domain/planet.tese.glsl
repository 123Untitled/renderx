...
void main(void) {
	...
	//vec3 interpoled_normal = (gl_TessCoord.x * in_normal[0])
	//					   + (gl_TessCoord.y * in_normal[1])
	//					   + (gl_TessCoord.z * in_normal[2]);
	//
	//// normalize normal
	//interpoled_normal = normalize(interpoled_normal);
	...
	const vec4 mpos = model.model * vec4(interpoled_position, 1.0f);
	// set distance to camera
	const float distance_to_camera = distance(cam.position, mpos.xyz);


	const float amplitude = 0.30f / (distance_to_camera + 0.01f);
	const float frequency = 5.0f / (distance_to_camera + 0.01f);

	// get noise
	const float noise = simple_noise(interpoled_position * frequency) * amplitude;
	...
	//vec3 world_normal = normalize(model.model * vec4(normal, 0.0)).xyz;
	...
	// output patch point view direction in world space
