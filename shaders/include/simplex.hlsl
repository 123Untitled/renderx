#ifndef ___noise_simplex___
#define ___noise_simplex___
...
/* fast floor */
static int _floor(const float fp) {
	const int i = int(fp);
	return (fp < i) ? (i - 1) : (i);
}
...
/* hash */
static uint _hash(const int i) {
	return _perms[uint(i) & 255U];
}

/* gradient */
static float _gradient(const int hash,
					   const float x,
					   const float y,
					   const float z) {
...
/* simplex */
static float simplex(const float x, const float y, const float z) {
...
/* fractal */
static float fractal(const uint octaves,
					 float amplitude,
					 float frequency,
	...
	for (uint i = 0U; i < octaves; ++i) {
		output += (amplitude * simplex(x * frequency,
									   y * frequency,
									   z * frequency));
		denom += amplitude;
